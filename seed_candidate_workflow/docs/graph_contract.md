...
Legacy CSVs may still use the column name `graph_run_id` with the same meaning; `seed_candidate_workflow.utils.pair_graph_contract` normalizes it to `graph_id` (`migrate_unscored_graph_id_column`, `ensure_unscored_contract`).
