#!/usr/bin/env python3
"""Step 5: PU scoring + community sweep for GNN + explicit pair features."""
...
def main() -> int:
    ...
    if not (args.skip_existing and sweep.is_file()):
        exp = resolve_repo_path(repo, str(manifest["community_experiment_gnn_plus"]))
        subprocess.run(
    ...
    out = steps_dir(repo, manifest) / "step05_community_gnn_plus_report.json"
