#!/usr/bin/env python3
"""Step 4: train thesis GNN-only pair scorer (ES100)."""
...
def main() -> int:
    ...
    if args.skip_existing and ckpt.is_file():
        print(f"[step04] skip (checkpoint exists): {ckpt}")
    else:
        from steps.train_stage import run_train_stage

        ref = resolve_repo_path(repo, str(manifest["reference_gnn_only_training_config"]))
        ...
        print(f"[step04] training run_id={run_id}")
        run_train_stage(
    ...
    out = steps_dir(repo, manifest) / "step04_train_gnn_only_report.json"
