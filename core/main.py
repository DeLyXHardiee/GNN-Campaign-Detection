...
def run_preprocessing():
    ...
    """
    from utils.remove_non_phishes import filter_phishing_emails
    
    input_csv = "../data/csv/TREC-07.csv"
    output_csv = "../data/csv/TREC-07-only-phishing.csv"
    
    print(f"Filtering phishing emails from {input_csv}...")
    phishing_count = filter_phishing_emails(input_csv)
    print(f"Filtered dataset saved to {output_csv} ({phishing_count} phishing emails)")

def run_trec_misp_converter(csv_path="../data/csv/TREC-07-only-phishing.csv", misp_json_path="../data/misp/TREC-07-misp.json"):
